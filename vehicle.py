...
class basicVehicle(BulletVehicle):
  def __init__(self,game,v_id,pos,vel,length,width,height,axisDis,wheelDis,radius,wheelH):
    ...
    if length < 17:
        ...
        v_type = 'bus'
    self.yugoNP = loader.loadModel('models/low_poly_cars_set/'+v_type+'.obj')
    nn = str(numpy.random.randint(3))
    tex = loader.loadTexture('models/low_poly_cars_set/tex/'+v_type+'_'+nn+'.png')
  ...
  # attach predictor to the vehicle
